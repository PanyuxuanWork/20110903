...
// -----------------------------------------------------------------------------
// Region final color mask
// -----------------------------------------------------------------------------
// _RegionColorMask.rgb = final cell color resolved on CPU.
// _RegionColorMask.a   = visible strength.
// In hard override mode, visible cells output Region RGB directly and Add Passes output 0.
// This makes region color independent from TerrainLayer count and splat composition.
TEXTURE2D(_RegionColorMask);
SAMPLER(sampler_RegionColorMask);

float _RegionEnabled;
float2 _RegionWorldMin;
float2 _RegionWorldSize;
float _RegionOverlayOpacity;
float _RegionHardOverride;

float2 WorldXZToRegionUV(float3 worldPos)
{
    float2 uv = (worldPos.xz - _RegionWorldMin.xy) / max(_RegionWorldSize.xy, 1e-5);
    return saturate(uv);
}

float4 SampleRegionColor(float3 worldPos)
{
    float2 uv = WorldXZToRegionUV(worldPos);
    return SAMPLE_TEXTURE2D(_RegionColorMask, sampler_RegionColorMask, uv);
}

float GetRegionAlpha(float4 region)
{
    return saturate(region.a * _RegionOverlayOpacity * step(0.5, _RegionEnabled));
}

float3 ApplyRegionOverlay(float3 litColor, float4 region)
{
    float alpha = GetRegionAlpha(region);

    if (alpha <= 1.0 / 255.0)
        return litColor;

    if (_RegionHardOverride >= 0.5)
        return region.rgb;

    return lerp(litColor, region.rgb, alpha);
}

float3 SuppressRegionAddPass(float3 addColor, float4 region)
{
    float alpha = GetRegionAlpha(region);
...
#endif
{
    ...
    // Region overlay is applied in forward shading only.
...
#else
    half4 region = (half4)SampleRegionColor(inputData.positionWS);
    ...
    // Important:
    // SplatmapFinalColor() multiplies color.rgb by color.a and applies terrain/add-pass fog.
    // Region final color must be applied AFTER that step, otherwise different TerrainLayer
    // combinations can still tint or dim the Region color.
    SplatmapFinalColor(color, inputData.fogCoord);

#if defined(TERRAIN_SPLAT_ADDPASS)
    color.rgb = (half3)SuppressRegionAddPass(color.rgb, region);
#else
    color.rgb = (half3)ApplyRegionOverlay(color.rgb, region);
#endif
